...
class AEModel(BaseModel):
    def __init__(self, network_params, act_out=tf.nn.softplus,
                 ...
                 summary_dir='', result_dir='', restore=False, plot=False, clustering=False, colab=False, model_type=0):
        ...
        with self.graph.as_default():
            if(model_type == const.AE):
                self.model_graph = AEGraph(network_params, act_out,
                                           transfer_fct, learning_rate, kinit, batch_size,
                                           reuse=False)
            if(model_type == const.AECNN):
                self.model_graph = AECNNGraph(network_params, act_out,
                                              transfer_fct, learning_rate, kinit, batch_size,
                                              reuse=False)
