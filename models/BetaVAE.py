"""BetaVAE.py:  Autoencoder methods"""
...
class BetaVAE(AE_BASE):
    def __init__(self, *argz, **kwrds):
        AE_BASE.__init__(self, *argz, **kwrds)
        self.config.model_name = 'BetaVAE'
        self.config.model_type = const.VAE
        self.setup_logging()
    ...
    def _build(self):
        ...
        ---------------------------------------------------------------------- '''
        from Alg_BetaVAE.BetaVAE_model import BetaVAEModel
        self.model = BetaVAEModel(network_params=self.network_params, act_out=utils.softplus_bias, sigma=self.config.sigma, beta=self.config.beta,
                              transfer_fct=tf.nn.relu, learning_rate=self.config.l_rate,
                              kinit=tf.contrib.layers.xavier_initializer(),
                              batch_size=self.config.batch_size, dropout=self.config.dropout,
                              ...
                              restore=self.flags.restore, plot=self.flags.plot, clustering=self.flags.clustering,
                              colab=self.flags.colab,
